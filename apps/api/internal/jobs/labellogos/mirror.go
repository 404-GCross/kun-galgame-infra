...
// dimsFileName is the manifest the crawler repos write at the mirror root: one
// JSON object per line, {"id","file","w","h","url"}.
const dimsFileName = "dims.jsonl"

// mirrorExts are the extensions a mirrored logo may carry, in probe order. The
// crawlers save whatever byte format the upstream served rather than
...
// filter: unlike the Bangumi cover lane (which needs w/h to tell a portrait
// from a landscape), a logo is used at whatever shape it has, and the image
...
//
// Both are tolerated absent. An empty root is the pre-crawl dry run, whose
// whole job is to size the population and emit the ids file — failing it for
// having no bytes would make the planning step impossible to perform. A missing
// dims.jsonl is tolerated for the same reason plus a stronger one: dims are not
// required to upload, so a manifest-less mirror is still a perfectly good
// mirror and this lane falls back to probing the documented layout.
//
...
func loadMirror(root string, src Source) (*mirror, error) {
	m := &mirror{root: root, src: src, entry: map[string]dimsEntry{}}
	if root == "" {
		return m, nil
	}
...
// resolve returns the local path holding this external id's logo bytes, if any.
// The manifest's `file` wins when it points at real bytes; otherwise the
// documented layout <root>/<external_id>/<stem>.<ext> is probed in mirrorExts
...
func (m *mirror) resolve(externalID string) (string, bool) {
	...
	for _, ext := range mirrorExts {
		p := filepath.Join(m.root, externalID, m.src.FileStem+"."+ext)
		if fileExists(p) {
			return p, true
		}
	}
...
// treated as absent: a truncated fetch is not bytes, and uploading it would
// burn the label's one slot on nothing.

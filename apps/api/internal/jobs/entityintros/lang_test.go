...
// TestDetectLang covers the mirrored step-57 heuristic's three-way outcome
// vocabulary: kana → ja, no kana → zh-Hans; the vndb lane never calls it
// (lang=en is lane-fixed).
...
func TestStripVNDBMarkupSpoiler(t *testing.T) {
	// A closed spoiler span disappears ENTIRELY, content included.
	...
	// Case-insensitive + multiple spans.
	in = "A [SPOILER]x[/SPOILER] B [spoiler]y[/spoiler] C"
	out, stripped = stripVNDBMarkup(in)
	assert.True(t, stripped)
	assert.Equal(t, "A  B  C", out)

	// An unclosed [spoiler] drops to the end — never leaks.
...
func TestStripVNDBMarkupLightTags(t *testing.T) {
	// [url=…] unwraps to the link text; [b]/[i] unwrap; attribution survives.
	...
	// No markup at all → verbatim (minus outer trim).
	out, stripped = stripVNDBMarkup("Plain description.")
	assert.False(t, stripped)
	assert.Equal(t, "Plain description.", out)
}

// TestStripVNDBMarkupEmptied: a description that is ONLY a spoiler must end
// empty (the caller counts it no_text and writes nothing).

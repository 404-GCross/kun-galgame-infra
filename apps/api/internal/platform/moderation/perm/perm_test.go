...
import (
	...
	"api/internal/platform/moderation/perm"
)

// goldenGrants is derived from the pre-migration admin/moderator route gate,
// with the +ren correction applied (the management-axis containment the old gate
// omitted). Any drift fails the build.
...
func TestCreatorGrantsNothing(t *testing.T) {
	for p := range goldenGrants {
		if perm.Resolver.Can([]string{"creator"}, p) {
			t.Errorf("creator must grant nothing on the moderation surface, but grants %q", p)
		}
	}
}

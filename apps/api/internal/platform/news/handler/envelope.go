...
// Kept byte-identical to the catalog face's mapping on purpose. huma.NewError is
// a package global and the news face is mounted in the catalog PROCESS, so both
// InstallErrorEnvelope calls write the same variable and the later one wins;
// were the two mappings to diverge, mount order would silently decide what the
// catalog face returns.

...
// S2S face: Basic client credentials only (backend-to-backend). Any valid
// first-party OAuth client authenticates; the tenant `site` is DERIVED from the
// client's binding (oauth_clients.catalog_site — the shared per-client site
// key), never taken from the request body, so a client can only meter/scan for
// its own site. This mirrors trust/community/image's S2S plumbing verbatim
// (charter ruling 9 — no new mechanism); each platform service keeps its own
// copy of this small helper rather than sharing one upstream.

const localClient = "ai:oauth_client"

type ctxKey string

const ctxKeyClient ctxKey = "ai:oauth_client"
...
func siteBinding(ctx context.Context) (string, *houseError) {
	...
	if client == nil || client.CatalogSite == "" {
		return "", apiErrMsg(http.StatusForbidden, errors.ErrForbidden,
			"client is not bound to a site; it cannot call the AI gateway")
	}

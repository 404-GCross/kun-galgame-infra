...
//
// ⚠️ RETIRED BY RULING (2026-08-08) — DO NOT POPULATE. It has never held a row,
// no label has ever carried an org_id, and it must stay that way. The job it was
// reserved for — "which company is this brand under" — is done by
// catalog_label_relation, which the corporate-graph spine
// (internal/jobs/orglabels/spine.go) now feeds for every company VNDB relates to
// anything, VISUAL ARTS and NEXTON included.
//
// The graph is not merely an alternative; org_id cannot do the job. It is one
// nullable FK, so it expresses ONE relation at ONE level, while the real
// vocabulary is eight relations at arbitrary depth — an imprint chain, a studio
// SPAWNED from another, a superseded trading name. spawned/formerly have nowhere
// to live here. Worse, it would make two kinds of company entity with two id
// spaces and two read faces, and then every company needs an answer to "org or
// label?" — for VISUAL ARTS, which publishes under its own name AND owns Key,
// the answer is "both", which is how a modelling question becomes permanent.
//
// The table and its FK are kept only because removing an entity type from the
// merge machinery (merge_service / survivorship / unmerge / snapshot /
// merge_rehang all switch on it) is a wave of its own, not because the design is
// still open. Anything that would "finally use catalog_org" is the thing this

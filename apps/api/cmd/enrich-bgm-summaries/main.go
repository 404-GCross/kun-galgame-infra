// enrich-bgm-summaries fills catalog_work_intro rows for BODYLESS galgame-
// doujin works from Bangumi summaries (BGM enrichment wave, refs/proj/57).
// The step-56a EXACT anchors (rule:bgm-title-year) point each work at a
...
// language (hiragana/katakana → 'ja', else 'zh-Hans') ONLY when the work has
// no intro row in that language yet (any source) — so a work whose DLsite ja
// intro already exists skips its duplicate ja summary. Logic lives in
// internal/jobs/bgmsummaries. Idempotent: a second --apply writes zero.
//
...
//
//	# dry-run: report zh_new / ja_fill / skip_dup_lang / no_summary + samples
...
//
//	# apply: write the fill-missing intro rows (source=bangumi)
//	go run ./cmd/enrich-bgm-summaries --apply \
//	    --dsn "host=127.0.0.1 port=5432 user=postgres password=... dbname=kun_catalog_rehearsal sslmode=disable"
package main

 ...
 * renders. Reads the two Tier-A public OpenAPI specs from the repo's docs/ (the
 * single source, produced by `cmd/gen-openapi` public targets), parses them,
...
// galgame 26 (W1a +/stats +/lookup = 12; W1b +14 taxonomy by-id ops) + catalog 20
// (prior 12 + A2-1b taxonomy lists 4 + A2-1c calendar buckets 3 + A2-1d
// works/search 1) = 46.
...
const buildFace = (faceDef) => {
  ...
  return {
    ...
    groups: [...groups.entries()].map(([tag, operations]) => ({
      tag,
      // e.g. "galgame-public" → "Galgame" (the -public suffix is redundant with

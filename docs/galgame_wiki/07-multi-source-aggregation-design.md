...
1. **`action='synced'`** 加进 `galgame_revision` CHECK。注意该 CHECK 是 `cmd/migrate-galgame` 里**显式 DROP/ADD**(见 `model/pr.go` 注释),要同时改**模型 struct tag** 与 **`cmd/migrate-galgame` 的 raw SQL**,否则旧库保留旧约束、INSERT 报 23514。
...
执行顺序(吸取上次教训):**先 `cmd/migrate-galgame` 加约束/表,再部署写 `action='synced'` 的新代码**,否则新代码 INSERT 撞缺失约束/表。

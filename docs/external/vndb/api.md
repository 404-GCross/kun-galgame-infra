...
A sandbox endpoint is available for testing and development at https://beta.vndb.org/api/kana, for more information see [the sandbox](https://beta.vndb.org/about-sandbox).
...
A HTTP 401 error is returned if the token is invalid. The [GET /authinfo](https://api.vndb.org/kana#get-authinfo) endpoint can be used validate and extract information from tokens.
...
Validates and returns information about the given [API token](https://api.vndb.org/kana#user-authentication). The JSON object has the following members:
...
Searching for and fetching database entries is done through a custom query format[1](https://api.vndb.org/kana#fn1). Queries are sent as `POST` requests, but I expect to also support the `QUERY` HTTP method once that gains more software support.
...
- filters
  ...
  Filters are used to determine which database items to fetch, see the section on [Filters](https://api.vndb.org/kana#filters) below.
...
- page
  ...
  Page number to request, starting from 1. See also the [note on pagination](https://api.vndb.org/kana#pagination) below.
...
| Name              | [F](https://api.vndb.org/kana#filter-flags) | Description                                                  |
| :---------------- | :------------------------------------------ | :----------------------------------------------------------- |
| `id`              | o                                           | vndbid                                                       |
...
| `release`         | m                                           | Match visual novels that have at least one release matching the given [release filters](https://api.vndb.org/kana#release-filters). |
| `character`       | m                                           | Match visual novels that have at least one character matching the given [character filters](https://api.vndb.org/kana#character-filters). |
| `staff`           | m                                           | Match visual novels that have at least one staff member matching the given [staff filters](https://api.vndb.org/kana#staff-filters). |
| `developer`       | m                                           | Match visual novels developed by the given [producer filters](https://api.vndb.org/kana#producer-filters). |
...
- relations.*
  ...
  All [visual novel fields](https://api.vndb.org/kana#vn-fields) can be selected here.
...
- tags.*
  ...
  All [tag fields](https://api.vndb.org/kana#tag-fields) can be used here. If you’re fetching tags for more than a single visual novel, it’s usually more efficient to only select `tags.id` here and then fetch (and cache) further tag information as a separate request. Otherwise the same tag info may get duplicated many times in the response.
...
- developers.*
  ...
  All [producer fields](https://api.vndb.org/kana#producer-fields) can be used here.
...
- staff.role
  ...
  String, see `enums.staff_role` in the [schema JSON](https://api.vndb.org/kana#get-schema) for possible values.
...
- staff.*
  ...
  All [staff fields](https://api.vndb.org/kana#staff-fields) can be used here.
...
- va.staff.*
  ...
  Person who voiced the character, all [staff fields](https://api.vndb.org/kana#staff-fields) can be used here.

- va.character.*

  VN character being voiced, all [character fields](https://api.vndb.org/kana#character-fields) can be used here.

- extlinks

  Array, links to external websites. Works the same as the ‘extlinks’ [release field](https://api.vndb.org/kana#release-fields).
...
| `official`          |                                             | See `patch`.                                                 |
| `has_ero`           |                                             | See `patch`.                                                 |
| `vn`                | m                                           | Match releases that are linked to at least one visual novel matching the given [visual novel filters](https://api.vndb.org/kana#vn-filters). |
| `producer`          | m                                           | Match releases that have at least one producer matching the given [producer filters](https://api.vndb.org/kana#producer-filters). |
...
In all of the above forms, an error is returned if the site is not known in the database or if the URL format is not recognized. The list of supported sites and URL formats tends to change over time, see [GET /schema](https://api.vndb.org/kana#get-schema) for the current list of supported sites.
...
- media.qty
  ...
  Integer, quantity. This is `0` for media where the quantity is unknown or where it does not make sense, like “internet download”.
...
- vns.*
  ...
  All [visual novel fields](https://api.vndb.org/kana#vn-fields) are available.
...
- producers.*
  ...
  All [producer fields](https://api.vndb.org/kana#producer-fields) are available.
...
- images.*
  ...
  All [visual novel](https://api.vndb.org/kana#vn-fields) `image.*` fields are available here as well.
...
| `extlink` | m                                           | Match on external links, works similar to the `extlink` filter for [releases](https://api.vndb.org/kana#release-filters). |
...
| `seiyuu`       | m                                           | Match characters that are voiced by the matching [staff filters](https://api.vndb.org/kana#staff-filters). Voice actor information is actually specific to visual novels, but this filter does not (currently) correlate against the parent entry when nested inside a visual novel filter. |
| `vn`           | m                                           | Match characters linked to visual novels described by [visual novel filters](https://api.vndb.org/kana#vn-filters). |

The `trait` and `dtrait` filters accept either a plain trait ID or a two-element array containing the trait ID and maximum spoiler level. These work similar to the tag filters for [visual novels](https://api.vndb.org/kana#vn-filters), except that traits don’t have a rating.
...
- image.*
  ...
  Object, possibly null, same sub-fields as the `image` [visual novel field](https://api.vndb.org/kana#vn-fields). (Except for `thumbnail` and `thumbnail_dims` because character images are currently always limited to 256x300px, but that is subject to change in the future).
...
- gender
  ...
  Possibly null, otherwise an array of two strings indicating the character’s non-spoiler gender and the character’s actual (spoiler) gender. Possible values are `null`, `"m"`, `"f"`, `"o"` (non-binary) or `"a"` (ambiguous).
...
- vns.*
  ...
  All [visual novel fields](https://api.vndb.org/kana#vn-fields) are available here.

- vns.release.*

  Object, usually null, specific release that this character appears in. All [release fields](https://api.vndb.org/kana#release-fields) are available here.
...
- traits.*
  ...
  All [trait fields](https://api.vndb.org/kana#trait-fields) are available here.
...
| `lang`    |                                             | Language.                                                    |
| `gender`  |                                             | Gender.                                                      |
| `role`    | m                                           | String, can either be `"seiyuu"` or one of the values from `enums.staff_role` in the [schema JSON](https://api.vndb.org/kana#get-schema). If this filter is used when nested inside a visual novel filter, then this matches the `role` of the particular visual novel. Otherwise, this matches the `role` of any linked visual novel. |
| `extlink` | m                                           | Match on external links, works similar to the `extlink` filter for [releases](https://api.vndb.org/kana#release-filters). |
...
| :---------- | :------------------------------------------ | :----------------------------------------------------------- |
| `id`        | o                                           | vndbid                                                       |
| `vn`        |                                             | Match quotes from the visual novel(s) described by [visual novel filters](https://api.vndb.org/kana#vn-filters). |
| `character` |                                             | Match quotes from the characters(s) described by [character filters](https://api.vndb.org/kana#character-filters). |
...
- vn.*
  ...
  Visual novel info, all [visual novel fields](https://api.vndb.org/kana#vn-fields) can be selected here.

- character.*

  Character info, all [character fields](https://api.vndb.org/kana#character-fields) can be selected here.

# List Management

## POST /ulist

Fetch a user’s list. This API is very much like `POST /vn`, except it requires the `"user"` parameter to be set and it has a different response structure. All [visual novel filters](https://api.vndb.org/kana#vn-filters) can be used here.
...
- releases.*
  ...
  All [release fields](https://api.vndb.org/kana#release-fields) can be selected here.
...
Slightly unintuitive behavior alert: this API *always* adds the visual novel to the user’s list if it’s not already present, and that also applies to the above “removing a vote” example. Use [DELETE](https://api.vndb.org/kana#delete-ulistid) if you want to remove a VN from the list.
...
- status
  ...
  Release status, integer. See `releases.list_status` in the [POST /ulist fields](https://api.vndb.org/kana#ulist-fields) for the list of possible values. Defaults to 0.
...
Remove a release from the user’s list. Returns success even if the release is not on the user’s list. Removing a release does not remove the associated visual novels from the user’s visual novel list, that requires separate calls to [DELETE /ulist/](https://api.vndb.org/kana#delete-ulistid).
...
- Add `sexual` field to [POST /trait](https://api.vndb.org/kana#post-trait).
...
- Add `gender` field to [POST /character](https://api.vndb.org/kana#post-character).

**2025-01-11**

- Add `gender` and `gender_spoil` filters to [POST /character](https://api.vndb.org/kana#post-character).

**2025-01-09**

- Add `extlink` filter and `extlinks` field to [POST /producer](https://api.vndb.org/kana#post-producer).

**2025-01-07**

- Add [POST /quote](https://api.vndb.org/kana#post-quote).

**2024-09-09**

- Add `images` field to [POST /release](https://api.vndb.org/kana#post-release).

**2024-07-06**

- Add `"n"` (sexless) as possible value for the `sex` field of [POST /character](https://api.vndb.org/kana#post-character).

**2024-06-05**

- Add `average` field to [POST /vn](https://api.vndb.org/kana#post-vn).

**2024-05-23**

- Add `extlinks` field to [POST /vn](https://api.vndb.org/kana#post-vn).

**2024-05-18**

- Add `va` field to [POST /vn](https://api.vndb.org/kana#post-vn).

**2024-05-11**

- Add `image{thumbnail,thumbnail_dims}` fields to [POST /vn](https://api.vndb.org/kana#post-vn). Beware: VN images can now be larger than 256x400px.

**2024-03-13**

- Add [POST /staff](https://api.vndb.org/kana#post-staff).
- Add `editions` and `staff` fields to [POST /vn](https://api.vndb.org/kana#post-vn).
- Add `enums.staff_role` and `extlinks./staff` members to [GET /schema](https://api.vndb.org/kana#get-schema).

**2023-11-20**

- Add `relations` field to [POST /vn](https://api.vndb.org/kana#post-vn).

**2023-08-02**

- Add `developers` field to [POST /vn](https://api.vndb.org/kana#post-vn).

**2023-07-11**

- Deprecated `popularity` sort options for [POST /ulist](https://api.vndb.org/kana#post-ulist) and [POST /vn](https://api.vndb.org/kana#post-vn), it’s now equivalent to sorting on the reverse of `votecount`.
- Deprecated `popularity` filter and field for [POST /vn](https://api.vndb.org/kana#post-vn).
...
- Add `voiced`, `gtin` and `catalog` fields to [POST /release](https://api.vndb.org/kana#post-release).

**2023-01-17**

- Add `listwrite` permission to API tokens.
- Add [PATCH /ulist/](https://api.vndb.org/kana#patch-ulistid).
- Add [PATCH /rlist/](https://api.vndb.org/kana#patch-rlistid).
- Add [DELETE /ulist/](https://api.vndb.org/kana#delete-ulistid).
- Add [DELETE /rlist/](https://api.vndb.org/kana#delete-rlistid).

------

1. Yes, sorry, I know every API having its own query system sucks, but I couldn’t find an existing solution that works well for VNDB.[↩︎](https://api.vndb.org/kana#fnref1)
2. Fun fact: the web interface also accepts filters in JSON form, but that tends to result in long and ugly URLs.[↩︎](https://api.vndb.org/kana#fnref2)
3. There is also a third representation for filters, which the API also accepts, but I won’t bother you with that. It’s only useful as an intermediate representation when converting between the JSON and string format, which you shouldn’t be doing manually.[↩︎](https://api.vndb.org/kana#fnref3)
...
- **来源/契约**:`F007` · C2 · ✅~(两审一致;auth 模型需团队再追 session-vs-oauth refresh token 是否同源)
...
- **来源/契约**:`F009` · C2 · ⚠️ 确认核心,**但标题"404"有误——实际是 500**
...
- **来源/契约**:`F010` + `GPT-M01` · C3 · ✅ 直读 `moemoepoint_service.go` 确认 · ⚠️ 并发 500 属**已文档化的接受取舍**,3 字段比较是**真契约缺口**
...
- **来源/契约**:`F040`(两审从 LOW 上调 MEDIUM)· C4 · ✅ 直读 `handler.go Meta` 确认 · ⚠️ 代码注释已自承"V2 再限制"
...
> 多为静默吞错 / 时区 / 一致性瑕疵。✅ 为本次直读确认,✅~ 为与两审一致 + 代码模式吻合。

| ID | 标题 | 位置 | 核实 |
|---|---|---|---|
| `F035` | daily-stats 用进程本地 TZ(`now.Location()`)定边界,与 PG `date_trunc('day')` 的会话 TZ 不一致 → 跨 TZ 部署少/多算一天 | `internal/platform/galgame/repository/admin_repository.go:62,73,119` | ✅ |
| `F037` | 密码重置(Forgot/Reset)不拒绝 banned/匿名化用户(与其它路径不一致;实际影响小,封禁者本就无法登录) | `internal/platform/auth/service/auth_service.go:482,524` | ✅ |
| `F038` | 改邮箱验证码用 `data.Code != code` 普通比较(非常量时间),与注册/邮箱验证路径的 `subtle.ConstantTimeCompare` 不一致 | `internal/platform/auth/service/auth_service.go:765`(对比 `:199`) | ✅ |
| `F039` | admin 用户搜索 `FindAllPaginated` 的 ILIKE 不转义 `% / _`(与 `SearchByName` 的 `escapeLikePattern` 不一致) | `internal/platform/auth/repository/user_repository.go:~290`(对比 `:80-107`) | ✅ |
| `F040`→见 M7 | (已上调 MEDIUM) | — | — |
| `F041` | 解压炸弹像素上限 `MaxDecodedPixels` 在**完整 `image.Decode` 之后**才校验,小文件可先分配巨图再被拒 | `internal/platform/image/processor/processor.go:36-43,57` | ✅~(应改用 `DecodeConfig` 先验尺寸) |
| `F042` | GC 某 phase 失败被计数但 runner 仍返回成功 | `internal/platform/image/service/gc.go` | ✅~ |
| `F043` | `MergePR` 的 `completed_time` 用事务前的 stale `galgame.Updated`(应为合并时刻) | `internal/platform/galgame/service/revision_service.go:448,451` | ✅(直读确认,对比 `DeclinePR:492` 用 `NOW()`) |
| `F044` | `MergePR` 内 contributor upsert 的 `Count`/`Create` 错误被吞 | `internal/platform/galgame/service/revision_service.go:~453` | ✅ |
| `F045` | `MergePR`/`Revert` 重放 snapshot 的 tag/official/engine/series id 时不重新校验存在性 | `internal/platform/galgame/service/revision_service.go` | ✅~ |
| `F046` | `GalgameRevision.action` / `GalgamePR.status` 的 CHECK 约束仅 inline gorm tag,AutoMigrate 无法 ALTER 既有约束(部署隐患) | `internal/platform/galgame/model/pr.go:12,45`(注释已自承) | ✅ |
| `F047` | galgame message 列表 `Count(&total)` 不检查 `.Error` → 可能 false zero | `internal/platform/galgame/...message...` | ✅~ |
| `F048` | `BanGalgamesByUser` 吞掉单项失败,接口报全成功无失败 id 列表 | `internal/platform/galgame/service/admin_service.go:179` | ✅~ |
| `F051` | `VerifyMoyuPassword` 手写"constant-time"字节循环,应用 `crypto/subtle` | (本次未在 `internal/platform/auth` 定位到该函数) | ❓ 需团队确认位置 |
| `F076` | `migrate-users` 重映射 oauth user id 后无 post-remap 对账断言(三库 id 集合一致性未校验) | `cmd/migrate-users/main.go` | ✅~ |
| `F082` | 两处 galgame 写路径在条件 `Create` 前的 contributor-existence `Count` 忽略 `.Error`(false-zero) | `internal/platform/galgame/...`(同 F044 模式) | ✅~ |
| `F087` | admin web 把 access_token 存在 JS 可读(非 httpOnly)cookie;最高权应用上的 XSS 可直接窃取 | `apps/web/app/composables/{useApiFetch,useAuth,useApi}.ts`(注释已自承非 httpOnly) | ✅(直读确认)⚠️ 属 SPA 读取令牌的设计取舍 |

---

## 跨仓但定位在本仓代码(migrate-users)

- **`F031`**(C1,两审定 LOW):kungal `system_message_read_state` 的 per-user 游标回填依赖运维**先跑 migrate-users 再跑 kungal 012**;`migrate-users` 的 FK 重映射清单**不含**该表(该表由 012 后建)。若顺序颠倒,游标指向 remap 前的旧 id。✅~ 修法二选一:把该表加进 `migrate-users` 的重映射清单,或让 kungal 012 的 id 源显式 + 幂等。

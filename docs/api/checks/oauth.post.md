...
| `POST /api/v1/auth/refresh` | 🌐 | `authH.Refresh` | ⏳ | 用 httpOnly refresh cookie 续 access_token |
| `POST /api/v1/auth/password/forgot` | 🌐 +strict | `authH.ForgotPassword` | ⏳ | 发重置邮件 |
| `POST /api/v1/auth/password/reset` | 🌐 +strict | `authH.ResetPassword` | ⏳ | 用 token 重置 |
...
| `POST /api/v1/oauth/clients` | ⚙️ | `siteH.CreateClient` | ⏳ | |

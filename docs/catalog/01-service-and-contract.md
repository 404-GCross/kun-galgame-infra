...
- 响应 `data`:`work`(id/medium/display_name/olang/content_rating/status/**site 认领态**)+ `titles`(official/alias/abbreviation/search_hint)+ `releases`(每个含 kind/模糊日期/各自 `anchors`)+ **`labels`(经 work_label 归属边,含 label 自身 kind + 归属 kind)**+ **`refs`**+ **`characters`(花名册投影)**+ **`intro`(多语言简介,媒体聚合读面)**+ **`covers`(封面集,含竖版,媒体聚合读面)**+ **`screenshots`(截图集,媒体聚合读面)**+ **`ratings`(评分集,源原生刻度,媒体聚合读面)**+ **`tags`(内容标签集,原样 folksonomy,媒体聚合读面)**+ **`popularity`(热度计数集,源原样计数,媒体聚合读面)**+ **`playtimes`(时长估计集,分钟归一,媒体聚合读面)**+ **`series`(系列归属,step 94)**。
...
- **`popularity` 块(热度计数集,媒体聚合读面 step 62;治理契约 refs/proj/62 + refs/proj/51 模板)**:统一形状 `[{ source_id, metric, value }]`,**一 (source, metric) 至多一元素**,按 `(source_id, metric)` 升序;claimed/bodyless 归并后消费者无感来源。**`metric` 词表(可扩,加常量即扩)**:`0`=downloads(购买/下载数,DLsite `dl_count`)、`1`=wishlist(收藏数,DLsite `wishlist_count`)、`2`=reviews(书面评论数,DLsite `review_count`);未来源(如 Bangumi favorite 五桶)追加新常量,**绝不改号**。**`value` 恒为源原样计数,绝不跨源相加**(DLsite 一笔销售与 Bangumi 一次收藏是不同单位)——消费端按 `(source_id, metric)` 分源渲染,与评分刻度规则同构。**空值语义**:源未公开的计数**无元素**(DLsite 商业作不公开 `dl_count`——缺席 ≠ 0),源公开的 0 是真实值、**有元素**(刷新回路保持其现势)。**桥接归并**:**claimed 作品**(`site='galgame_wiki'`)热度**读时桥接**自 `galgame_dlsite_meta` 三计数列(dl_count→0 / wishlist_count→1 / review_count→2,`source_id=dlsite`;NULL 列无行);**绝不复制**进 catalog 原生行(bridge-not-copy)。**bodyless 作品**(`site=''`)读 `catalog_work_popularity` 原生行(UNIQUE (work_id, source_id, metric);62 回填:DLsite exact **release** 锚 × DLsite mirror `info_json`,game 域限定,ASMR 裁定出局)。**严格 XOR**:claimed 只读桥接;meta 无任何公开计数 = `popularity` 空 `[]`,**不回落**原生行(即便存在被遮蔽的原生行也不读——shadow-never-delete §8.B)。**易变值刷新**:两侧写路径均为值变才更的 `ON CONFLICT DO UPDATE`(claimed 侧 `cmd/enrich-dlsite-meta`、bodyless 侧 `cmd/backfill-work-ratings` DLsite 泳道)——kun-dlsite-api `refresh` 更新镜像 → 重跑即刷新。批量:claimed 一批 `galgame_dlsite_meta` + bodyless 一批 `catalog_work_popularity`,非逐 work。空序列化 `[]`。纯 DB 零字节——无 refping 事。

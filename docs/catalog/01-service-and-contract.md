...
- **`ratings` 块(评分集,媒体聚合读面 step 58a;治理契约 refs/proj/58 Facet A + refs/proj/51 模板)**:统一形状 `[{ source_id, score, vote_count, rank? }]`,**一源至多一元素**,按 `source_id` 升序;claimed/bodyless 归并后消费者无感来源。**score 恒为源原生刻度,绝不归一**(58 拍板):`vndb` = 1-10 **均值**、`bangumi` = 0-10 **均值**、`erogamespace` = 0-100 **中央值**——语义不同,消费端按 `source_id` 分源渲染(kungal 现行即分源展示 BGM/EG 分)。`rank` = 源内排名,源无此概念(VNDB meta/EG)或该作未入榜(Bangumi rank 0)时缺省。**桥接归并**:**claimed 作品**(`site='galgame_wiki'`)评分**读时桥接**自三张窄 meta 表(step 10/42/60 产物,与 catalog 同库):`galgame_vndb_meta`(rating/vote_count→`{source_id=vndb, score=rating/10, vote_count}`;rating 存 kana 线上刻度 10-100,÷10 即 VNDB 自身展示的 1-10 刻度——同源解码非跨源归一;`rating IS NULL` 滤除——无公开评分,sync 绝不写假 0)∪ `galgame_bangumi_meta`(score/rank/total→`{source_id=bangumi, score, vote_count=total, rank}`;`score<=0`=未评分,滤除)∪ `galgame_eg_meta`(median/vote_count→`{source_id=erogamespace, score=median, vote_count}`;`median IS NULL` 滤除——EG 的 0 是真实低分,NULL 才是无分);**绝不复制**进 catalog 原生行(bridge-not-copy)。**bodyless 作品**(`site=''`)读 `catalog_work_rating` 原生行(UNIQUE (work_id, source_id),58a 回填:BGM exact 锚 × `src_bangumi.subject` + EG exact 锚 × EG mirror)。**严格 XOR**:claimed 只读桥接;meta 全部无分 = `ratings` 空 `[]`,**不回落**原生行(即便存在被遮蔽的原生行也不读——shadow-never-delete §8.B)。批量:claimed 每 meta 表一批 + bodyless 一批 `catalog_work_rating`,非逐 work。空序列化 `[]`。纯 DB 零字节——无 refping 事。

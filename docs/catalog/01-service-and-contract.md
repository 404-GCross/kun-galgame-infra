...
- **`intro` 块(多语言简介,媒体聚合读面 step 52;治理契约 refs/proj/51)**:统一形状 `[{ lang, intro, source_id, machine? }]`,一语言一元素,claimed/bodyless 归并后消费者无感来源。**桥接归并**:**claimed 作品**(`site='galgame_wiki'`)简介**读时桥接**自 galgame body 的四固定语言列 `galgame.intro_{en_us,ja_jp,zh_cn,zh_tw}`(列→行 pivot,lang=`en`/`ja`/`zh-Hans`/`zh-Hant`),`source_id`=**galgame_wiki**(catalog_source 12);**绝不复制**进 catalog 原生行(bridge-not-copy)。**bodyless 作品**(`site=''`)读 `catalog_work_intro` 原生行,一语言多源时**先按 `provenance` 升序(源行 0 优先于机翻行 1)再按 `source_id` 升序**取一(user > vndb > …)。**严格 XOR**:claimed 作品只读桥接;galgame 简介全空 = `intro` 空 `[]`,**不回落**原生行(即便存在被遮蔽的原生行也不读——shadow-never-delete)。`source_id` 引用 `catalog_source` 注册表(归因,§8.C)。批量:读面按 claimed/bodyless 分两批查(galgame 一批 pivot + catalog_work_intro 一批),非逐 work。空序列化 `[]`。
  - **`machine` 标记(机翻试点 step 75,refs/proj/75)**:`catalog_work_intro` 的机翻行(`provenance=1`)在读面元素上带 `machine: true`,源/桥接行**省略**该字段——机翻文本**永不冒充源数据**,消费端可渲染「机翻」徽标。机翻行**填缺语言**落地(ja→zh-Hans 试点):仅当该 work **无任何 `zh-Hans/zh-Hant` 源行**(`provenance=0`)时写;它沿用源 ja 行的 `source_id`(归因=译自该源),另记 `src_hash=sha256(源 ja 文本)`(源文变则重译)与 `mt_model`(问责)。新源/人工 zh 落地后,上面的 provenance 升序令读面**自动优先源行**、机翻行退居遮蔽(shadow-never-delete)。源料:step 52 VNDB `vn.description`(en)、step 55 DLsite、step 57 Bangumi summary(原样存,不清洗)。
...
- **`screenshots` 块(截图集,媒体聚合读面 step 54,媒体聚合三读面波收官;治理契约 refs/proj/51)**:统一形状 `[{ image_hash, caption, sort_order, sexual, violence, source_id }]`,一截图一元素,claimed/bodyless 归并后消费者无感来源;按 `(sort_order, image_hash)` 排序。与 `covers` 同构,差异:截图带 `caption`、无 `kind`/`portrait_pinned`。**桥接归并**:**claimed 作品**(`site='galgame_wiki'`)截图**读时桥接**自 `galgame_screenshot`(caption/sexual/violence/sort_order 原样),`source_id` 由 `galgame_screenshot.source` 文本映射到 `catalog_source`:`''`→**galgame_wiki**(12,wiki 用户上传=首方 body)、`vndb`→vndb(2)(截图源域仅此两值;未知值兜底 galgame_wiki);**绝不复制**进 catalog 原生行,桥接读**不重传字节**(galgame 截图永住 galgame_wiki image scope)。**bodyless 作品**(`site=''`)读 `catalog_work_screenshot` 原生行(字节住 catalog image scope)。**严格 XOR**:claimed 只读桥接;galgame 截图空 = `screenshots` 空 `[]`,**不回落**原生行(即便存在被遮蔽的原生行也不读——shadow-never-delete §8.B)。批量:claimed 一批 `galgame_screenshot` + bodyless 一批 `catalog_work_screenshot`,非逐 work。空序列化 `[]`。**字节纪律 §4**:catalog scope 图(bodyless 截图/封面+立绘)由 `catalog-image-refping` 保活,其全集含 `catalog_work_screenshot` **全部行(含被遮蔽行)**,漏计遮蔽行 = GC 吃活图(66k 同类);galgame scope 图(claimed 桥接截图)由 `galgame-image-refping` 独立保活。**bodyless 截图数据**:EG 零字节、DLsite(6,102 release 锚)样图在外部 CDN 需独立字节波——本波只交付 schema+读面+refping 管线,bodyless 截图数据留待 55 号 DLsite 字节波(诚实,不硬凑)。
- **`ratings` 块(评分集,媒体聚合读面 step 58a;治理契约 refs/proj/58 Facet A + refs/proj/51 模板)**:统一形状 `[{ source_id, score, vote_count, rank? }]`,**一源至多一元素**,按 `source_id` 升序;claimed/bodyless 归并后消费者无感来源。**score 恒为源原生刻度,绝不归一**(58 拍板):`vndb` = 1-10 **均值**、`bangumi` = 0-10 **均值**、`dlsite` = 0-5 **星均值**(两位小数,DLsite 自身展示刻度;镜像线上键 `rate_average_star` 是该值的半星分桶 ×10 星件编码,不采)、`erogamespace` = 0-100 **中央值**——语义不同,消费端按 `source_id` 分源渲染(kungal 现行即分源展示 BGM/EG 分)。`rank` = 源内排名,源无此概念(VNDB meta/DLsite/EG)或该作未入榜(Bangumi rank 0)时缺省。**桥接归并**:**claimed 作品**(`site='galgame_wiki'`)评分**读时桥接**自四张窄 meta 表(step 10/42/60/62 产物,与 catalog 同库):`galgame_vndb_meta`(rating/vote_count→`{source_id=vndb, score=rating/10, vote_count}`;rating 存 kana 线上刻度 10-100,÷10 即 VNDB 自身展示的 1-10 刻度——同源解码非跨源归一;`rating IS NULL` 滤除——无公开评分,sync 绝不写假 0)∪ `galgame_bangumi_meta`(score/rank/total→`{source_id=bangumi, score, vote_count=total, rank}`;`score<=0`=未评分,滤除)∪ `galgame_dlsite_meta`(rate_average_star/rate_count→`{source_id=dlsite, score, vote_count}`;`rate_average_star IS NULL` 或 `rate_count<=0` 滤除——DLsite 评分不足 ~5 票不公开,镜像键整体缺省,0 票星数是空值语义,数据核实:星数与票数恒同现同缺,最小观测票数 5)∪ `galgame_eg_meta`(median/vote_count→`{source_id=erogamespace, score=median, vote_count}`;`median IS NULL` 滤除——EG 的 0 是真实低分,NULL 才是无分);**绝不复制**进 catalog 原生行(bridge-not-copy)。**bodyless 作品**(`site=''`)读 `catalog_work_rating` 原生行(UNIQUE (work_id, source_id),58a+62 回填:BGM exact 锚 × `src_bangumi.subject` + EG exact 锚 × EG mirror + DLsite exact **release** 锚 × DLsite mirror,game 域限定)。**易变值刷新(62)**:回填写路径统一为 `ON CONFLICT DO UPDATE`(值变才更,行级 `IS DISTINCT FROM` 比对)——镜像刷新后重跑工具即全量刷新,未变镜像重跑零更新。**严格 XOR**:claimed 只读桥接;meta 全部无分 = `ratings` 空 `[]`,**不回落**原生行(即便存在被遮蔽的原生行也不读——shadow-never-delete §8.B)。批量:claimed 每 meta 表一批 + bodyless 一批 `catalog_work_rating`,非逐 work。空序列化 `[]`。纯 DB 零字节——无 refping 事。
- **`tags` 块(内容标签集,媒体聚合读面 step 58b + T2;治理契约 refs/proj/58 Facet B + refs/proj/51 模板 + refs/proj/70 §3/§8)**:统一形状 `[{ name, count?, source_id, canonical_id?, tier?, kind? }]`,一 tag 名一源一元素,按 `(count DESC, name)` 排序(高票在前);桥接 ∪ 原生归并后消费者无感来源。**name 恒为源原文,原生层不做词表映射/规范化**(58 拍板;仅 trim 首尾空白)——规范层(step 74)在其上加**加法覆盖**(见下),原生 name/count/source_id 永不改写。`count` = 该 tag 在源上的票数,源无票数概念时**缺省**(整个 galgame wiki tag 层无票数;bangumi moderated meta_tags 亦 count=0)。**内容标签与 `labels` 严格分离**:`labels` 是归属词汇(组织责任,brand/publisher/circle/group),**tags 绝不入 `catalog_label`**(词汇红线)。**双泳道归并((facet, source) XOR,T2 修正,取代旧的整 facet XOR)**:对 tags facet,**每 (work, source) 只有一条泳道**——**wiki 桥接泳道**(`galgame_wiki`=用户自建、`vndb`=sync)恒**读时桥接**、绝不物化;**catalog 原生泳道**(`bangumi`、`dlsite`)恒读 `catalog_work_tag` 原生行;两泳道各带 `source_id` 归因,claimed 读面 = 桥接 ∪ 原生。**wiki 桥接泳道(仅 claimed)**:**claimed 作品**(`site='galgame_wiki'`)标签读时桥接自 `galgame_tag_relation ⋈ galgame_tag`(kungal 在用的 wiki tag 层;`galgame_tag.name` 即展示名——VNDB sync 经 tagMap 英→中本地化,未映射者保留英文原名,用户自建者原样),**仅出非剧透档**(`spoiler_level=0`——统一形状无 spoiler 位,桥出剧透 tag 会无标记泄露;galgame 读面自带 spoiler_level 可自行门控,此面不能),`source_id` 由 `galgame_tag_relation.source` 文本映射:`''`→**galgame_wiki**(12,用户自建)、`vndb`→vndb(2,sync 同步;未知值兜底 galgame_wiki);**绝不复制**进 catalog 原生行(bridge-not-copy,§2 不变)。**catalog 原生泳道(所有作品,claimed 与 bodyless 一视同仁)**:读 `catalog_work_tag` 原生行(UNIQUE (work_id, name, source_id);58b + 71 回填:BGM exact 锚 × `src_bangumi.subject` folksonomy tags(原样全存,不设票数阈值,count=1 也存,消费端按 count 过滤渲染)+ moderated meta_tags(count=0));**T2 放开 claimed**——bgm/dlsite 是 catalog 原生源,claimed 的 bgm tag 不再因认领态隐没,与 bodyless 同走原生泳道。**(facet, source) XOR 语义**:wiki 源恒桥接不物化、catalog 源恒原生,故 **claimed 读面 = 桥接 ∪ 原生归并后按 `(count DESC, name)` 重排**(有票 bgm 行天然在前,桥接 count=0 行按名序在后),bodyless 是同一规则的退化情形(无 wiki 体 → 桥接空集,只余原生)。旧「claimed 只读桥接、不回落原生」收窄为 per-source 事实:claimed 的 wiki 源不入原生表(bridge-not-copy),但其 catalog 源原生行正是读面一等公民。**popularity facet 已同规(T2b,refs/proj/102):claimed 的 bgm 五桶原生行同走原生泳道出面,dlsite 仍桥接独占**。批量:claimed 一批 bridge join + 全部作品一批 `catalog_work_tag`,非逐 work。空序列化 `[]`。纯 DB 零字节——无 refping 事。
...
- **`popularity` 块(热度计数集,媒体聚合读面 step 62;治理契约 refs/proj/62 + refs/proj/51 模板)**:统一形状 `[{ source_id, metric, value }]`,**一 (source, metric) 至多一元素**,按 `(source_id, metric)` 升序;claimed/bodyless 归并后消费者无感来源。**`metric` 词表(可扩,加常量即扩)**:`0`=downloads(购买/下载数,DLsite `dl_count`)、`1`=wishlist(收藏数,DLsite `wishlist_count`)、`2`=reviews(书面评论数,DLsite `review_count`);`10`=bgm_wish、`11`=bgm_collect(dump 键 "done")、`12`=bgm_doing、`13`=bgm_on_hold、`14`=bgm_dropped(Bangumi favorite 五桶,71/T3 回填;T2b/102 对 claimed 开放);未来源追加新常量,**绝不改号**。**`value` 恒为源原样计数,绝不跨源相加**(DLsite 一笔销售与 Bangumi 一次收藏是不同单位)——消费端按 `(source_id, metric)` 分源渲染,与评分刻度规则同构。**空值语义**:源未公开的计数**无元素**(DLsite 商业作不公开 `dl_count`——缺席 ≠ 0),源公开的 0 是真实值、**有元素**(刷新回路保持其现势)。**双泳道归并((facet, source) XOR,T2b 修正,取代旧的整 facet XOR)**:对 popularity facet,**每 (work, source) 只有一条泳道**——**dlsite 桥接泳道**恒**读时桥接**、绝不物化:**claimed 作品**(`site='galgame_wiki'`)的 dlsite 计数读时桥接自 `galgame_dlsite_meta` 三计数列(dl_count→0 / wishlist_count→1 / review_count→2,`source_id=dlsite`;NULL 列无行),**绝不复制**进 catalog 原生行(bridge-not-copy);claimed 的原生 dlsite 行(shadow-never-delete 遗留)**永不出面**。**catalog 原生泳道(bgm 五桶,claimed 与 bodyless 一视同仁)**:读 `catalog_work_popularity` 原生行(UNIQUE (work_id, source_id, metric);62 回填:DLsite exact **release** 锚 × DLsite mirror `info_json`,game 域限定,ASMR 裁定出局;71 回填 + T2b/102 解禁:BGM exact **work** 锚 × `src_bangumi.subject.favorite` 五桶,claimed 与 bodyless 同写)。**claimed 读面 = dlsite 桥 ∪ 原生非 dlsite 行,合并后按 `(source_id, metric)` 升序重排**(bgm=3 先于 dlsite=4);bodyless 是退化情形(无 galgame 体 → 桥接空集,只余原生行,含原生 dlsite 行)。meta 无公开计数且无原生非 dlsite 行 = `popularity` 空 `[]`。**易变值刷新**:两侧写路径均为值变才更的 `ON CONFLICT DO UPDATE`(dlsite 桥侧 `cmd/enrich-dlsite-meta`、原生 dlsite 行 `cmd/backfill-work-ratings`、bgm 五桶 `cmd/backfill-bgm-work-meta`)——kun-dlsite-api `refresh` 更新镜像 → 重跑即刷新。批量:claimed 一批 `galgame_dlsite_meta` 桥 + 两泳道各一批 `catalog_work_popularity`(claimed 侧排除 dlsite 源),非逐 work。空序列化 `[]`。纯 DB 零字节——无 refping 事。

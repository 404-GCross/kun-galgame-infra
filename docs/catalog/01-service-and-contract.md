...
- **粒度 = 一人一作一票**(唯一键 `(work_id, actor_uid)`,非 per-cover):`PUT` 到另一张封面 = 该票**搬家**(`ON CONFLICT DO UPDATE`),不是再投一票。响应 `data`:`{ cover_id, vote_count, voted }`——`vote_count` 是本次写之后该封面的票数,免二次读。
- `DELETE` 撤票:**幂等**(本就没票也是 200,不是 404),路径对称携带 `coverID`(用户一作至多一票,故该 id 不必与所投的那张相符);响应同形,`voted=false`。撤票**不校验作品可投性**——作品不再 live 不该困住它 live 时收下的票。
- 拒绝:`actor.user_id<=0` → 400(服务侧;wire 上先被 schema 闸判 422)· 作品不存在/软删/非 live(如已合并) → 404 · 封面不存在或不属于该作品 → 404(两者合一,调用方下一步都是重读封面集)· client 站点绑定不符 → 403。
- 读回:票数与 `voted` 挂在 §2.4 的 `covers[]` 上(`?uid=` 才出 `voted`)。**级联**:`catalog_cover_vote.cover_id` 带 `ON DELETE CASCADE` 外键——封面行一删,其票自动消失,无需任何 Go 代码记得这件事。
- **已知副作用(诚实记录)**:编辑面的 covers 字段是**整行集替换**(`editspec` 先删该作品全部 `source=curated` 封面行再重建,行 id 随之换新),因此**一次封面编辑会连带清空这些封面上的票**(非 curated 源的 vndb/bangumi/dlsite 行不受影响)。票是参考数据、重投成本低,故本波按级联语义如实接受;若日后要让票熬过编辑,正解是把编辑面改成**按 image_hash 就地 upsert**(保 id),而不是给票表加旁路。

...
由 `cmd/aggregate-user-playtime` 离线产出,**没有任何用户写入能同步移动已发布的数字**。只统计 `finished`;先按用户把各 client 折成 MAX 再对用户取中位数(`percentile_disc`,真实观测值,与 vndb / erogamespace 同语义);**少于 3 个完成者不发布且删行**(用户 2026-08-09 裁定)—— 两个人定义的中位数比没有更坏,跌破阈值必须停发而不是冻结。计入还要过 `minutes ∈ [10, 60000]`。**这些闸只管投票权不管存储**:用户自己的行原样存、原样读得回来。

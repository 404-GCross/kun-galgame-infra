...
- **`tags` 块(内容标签集,媒体聚合读面 step 58b;治理契约 refs/proj/58 Facet B + refs/proj/51 模板)**:统一形状 `[{ name, count?, source_id, canonical_id?, tier?, kind? }]`,一 tag 名一源一元素,按 `(count DESC, name)` 排序(高票在前);claimed/bodyless 归并后消费者无感来源。**name 恒为源原文,原生层不做词表映射/规范化**(58 拍板;仅 trim 首尾空白)——规范层(step 74)在其上加**加法覆盖**(见下),原生 name/count/source_id 永不改写。`count` = 该 tag 在源上的票数,源无票数概念时**缺省**(整个 galgame tag 层无票数)。**内容标签与 `labels` 严格分离**:`labels` 是归属词汇(组织责任,brand/publisher/circle/group),**tags 绝不入 `catalog_label`**(词汇红线)。**桥接归并**:**claimed 作品**(`site='galgame_wiki'`)标签**读时桥接**自 `galgame_tag_relation ⋈ galgame_tag`(kungal 在用的 wiki tag 层;`galgame_tag.name` 即展示名——VNDB sync 经 tagMap 英→中本地化,未映射者保留英文原名,用户自建者原样),**仅出非剧透档**(`spoiler_level=0`——统一形状无 spoiler 位,桥出剧透 tag 会无标记泄露;galgame 读面自带 spoiler_level 可自行门控,此面不能),`source_id` 由 `galgame_tag_relation.source` 文本映射:`''`→**galgame_wiki**(12,用户自建)、`vndb`→vndb(2,sync 同步;未知值兜底 galgame_wiki);**绝不复制**进 catalog 原生行(bridge-not-copy)。**bodyless 作品**(`site=''`)读 `catalog_work_tag` 原生行(UNIQUE (work_id, name, source_id);58b 回填:BGM exact 锚 × `src_bangumi.subject.tags` folksonomy 原样全存,不设票数阈值,count=1 也存,消费端按 count 过滤渲染)。**严格 XOR**:claimed 只读桥接;galgame tag 全空(或全为剧透档)= `tags` 空 `[]`,**不回落**原生行(即便存在被遮蔽的原生行也不读——shadow-never-delete §8.B)。批量:claimed 一批 join + bodyless 一批 `catalog_work_tag`,非逐 work。空序列化 `[]`。纯 DB 零字节——无 refping 事。

...
> - **新增 `covers[]`**：作品的封面候选集，按 `image_hash` 引用 image_service（无跨服务 FK；写入前下游须经 wiki 的 [`POST /galgame/image`](./01-galgame.md#galgame-图片上传post-galgameimage) 上传拿 hash——由 wiki 以 `site=galgame_wiki` 代传，**下游不得用自己的 image_service client 直传 galgame 图片**，否则字节落到 `site=kungal`/`site=moyu`、站点级 refping 保不活）。每条 `{image_hash, sort_order, sexual, violence, source, source_key}`。**`sort_order=0` = "钉住的封面"，DB 强制约束每作品最多一张**（partial unique index）。管理员"换封面"= 在同一编辑请求里把旧 cover 的 `sort_order` 改为 1 + 新 cover 的 `sort_order` 改为 0（**绝不**在两次请求里分别做，否则中间态会被 DB 拒绝）。

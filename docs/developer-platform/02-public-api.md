...
| `GET /v1/catalog/search` | `catalog:read` | **实体自动补全**(`type=names\|characters\|labels\|works\|tags`,五索引;**`tags` 为 A2-1d 加法**,hit 镜像 labels 惯例并附 `tier`/`kind`)。**🔴 wave 209 改形**(此前的「hit shape 逐字节冻结」就此解除):`name` → `display_name`;name/character/label 类 hit 补 `localized{}`、works 类 hit 补 `names` 块——中文界面的搜索下拉从此不用二次查详情,见 §3.7 wave 209 节。至多 20 条扁平 hit、无过滤无分页 —— picker / 跳转框用面;**作品结果页(过滤/facets/排序/翻页/完整列表行)走 `GET /v1/catalog/works/search`** |
...
- 该旗标同时出现在详情面 `intro[]`、列表 `include=intros` 的每个槽,以及 `characters/{id}` / `labels/{id}` / `names/{id}` / `tags/{id}` 的 `intros[]`,语义逐字一致——**wave 209 起五个 intro 面在 spec 层就是同一个 schema(`PublicIntro`)**,消费端写一个渲染器即可;tag 面的 `machine` 现阶段**恒 false**(`catalog_tag_intro` 无 provenance 列,写入方只有 curated 编辑面)。
...
- **加法**:`PublicWorkBrief`(relations / series_siblings / lookup / tag·label·name·character 的 works 子列表)与 works 类 search hit 补 `names` 块(与 works 列表 `include=names` 同一选举);name/character/label 类 hit 补 `localized{}`。

...
>
> **锚存活性:上游删条目 ≠ 我们删锚(wave 207 补文)**:上游把某条记录删掉后,我们**不删这条 `catalog_external_ref` 行**,只给它打 `dead_at` 时间戳——删了下一轮导入会照规则把同一条锚重新推回来(负知识必须留档,同 `catalog_match_rejection` 之理)。**公开面的 `refs[]` 不渲染已死锚**:works 详情、works 列表 `include=refs`,以及 names / characters / labels / engines / series 各面同规;导入 / 审计 / S2S 侧仍看得见它。锚复活(上游又出现)时同一轮把 `dead_at` 清回 `NULL`,该行原地恢复渲染。当前唯一的写入方是周跑的 `cmd/audit-vndb-anchors`(只管 **vndb × work × exact** 这一族),它带 `--min-mirror-rows` 地板——镜像半载时**拒绝执行**,否则会把整批活锚一次判死。
>

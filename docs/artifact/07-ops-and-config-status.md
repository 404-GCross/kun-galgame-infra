...
> 本篇是 artifact 服务「**还差什么、什么还没配**」的**唯一权威清单**。每完成一项就勾掉。最后更新：2026-06-13。
...
1. **配 B2（或任意 S3 兼容）凭证**到 `apps/api/.env`（已留注释模板）：`KUN_ARTIFACT_S3_ENDPOINT/REGION/ACCESS_KEY/SECRET_KEY/BUCKET`，`FORCE_PATH_STYLE`（B2=false / MinIO=true）。
   - 配好后：`go run ./cmd/artifact` 应能启动（启动时 `EnsureBucket` + AutoMigrate），`GET /healthz` 返回 ok。
   - 临时省事也可复用现有 image dev 的 R2 凭证（`.env` 里的 `KUN_IMAGE_S3_*`）指向一个 `kun-artifacts-dev` 桶——但建议单独的桶/密钥，避免与图床混。
2. 开 `KUN_ARTIFACT_UPLOAD_ENABLED=true` 后端到端验证 init→PUT→complete→download。
   - 前端直传还需在桶上配 **CORS**（允许来源 `PUT`/`GET`，`exposeHeaders` 含 `ETag`）。

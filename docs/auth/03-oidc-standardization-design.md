...
- [ ] **Migrate `kun_galgame_infra` BEFORE pushing these commits**: `go run ./cmd/migrate`
  (adds `signing_keys` + `authorization_codes.nonce`). Deploy does NOT auto-run it, and a
  push auto-deploys via CI/Dokploy. This is **flag-independent**: the new oauth binary
  INSERTs `authorization_codes.nonce` on **every consent** — deploying against an
  unmigrated DB 500s ALL SSO logins immediately (Postgres 42703). The migration is
  additive and harmless under the old binary, so the safe order is always:
  **migrate → push → verify**.

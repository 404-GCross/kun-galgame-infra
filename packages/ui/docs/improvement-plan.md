...
### 批 1（修 latent bug + 颜色泄漏，不改 API；1-2 天）

- [ ] §1.1 Tab 动态类替换为静态映射表（保持现有 variant 不变）
- [ ] §1.1 Progress.vue gradient variant 同款修复
- [ ] §1.3 颜色泄漏：Input / Textarea / Select / Popover / Tooltip / DatePicker / Pagination 全部 red → danger，bg-white/black → bg-content1
- [ ] §3.13 DatePicker `getMonth()` 0-indexed bug
- [ ] §3.18 Avatar 死 props 删除（`floatingPosition` / `disableFloating`）
- [ ] §3.21 Link `:is="tag"` 修复

**风险**：低，全部是内部实现修正，调用方零感知。

### 批 2（KunUI v0.1.0，API breaking；3-5 天）

- [ ] §1.2 抽 `kunVariantClasses` 共享映射
- [ ] §1.4 `info` 加进中央 `KunUIColor`（同步 tailwindcss.css 补 info 调色板）
- [ ] §1.5 size 类型统一 `KunUISize`
- [ ] §1.6 Modal `modalValue` → `modelValue` + 仓内 12 个调用点一并改
- [ ] §1.7 删 5 个 KunLucide* + MessageItem 改 KunIcon
- [ ] §1.8 全员 `defineModel`
- [ ] **§4 KunTab 重做**（新 5 variant + 滑动指示器 + 键盘 + vertical）
- [ ] **§5 KunTagInput 新建**
- [ ] §3.14 Badge → Chip 重命名 + 新建 Badge（dot/count）
- [ ] §3.16 Card 拆 `clickable` vs `href`
- [ ] §3.19 Brand 抽 props（或移出 @kun/ui）
- [ ] §3.20 alert/Info → alert/Loli + 合并 Info / info/Info
- [ ] §3.28 GlassShatter 决定恢复 or 删除

**风险**：中高，10+ 处 API 变更，但 v0.0.1 → v0.1.0 是合理时机，集中一次破坏比每月小破坏好。

...
- The **source of truth for these contracts is in the code** (`cmd/oauth`, `cmd/image`, `cmd/galgame`, `cmd/artifact`, `cmd/catalog`, and other handlers) — when you change the code, change the source docs here in the same PR; `docs:verify` will catch "docs that don't match the reality of the code".

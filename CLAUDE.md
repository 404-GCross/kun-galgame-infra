...
- galgame (wiki-family) models **and** catalog models → `go run ./cmd/migrate-catalog` — the **single entry point for both families since W5**: it migrates the galgame models against `KUN_GALGAME_PG_DATABASE` and the catalog models against `KUN_CATALOG_PG_DATABASE` over two pools — **both point at `kun_catalog` everywhere since 2026-07-29** (the dev split-DB posture retired; the standalone `kun_galgame_wiki` database no longer exists in dev, and prod's copy is a frozen W1 rollback remnant pending drop). Prod runs it on every deploy (compose `depends_on`), but for outage-class schema changes still follow the manual migrate-first order below.

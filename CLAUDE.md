...
`docs/integration/oauth`, `docs/image_service`, `docs/integration/galgame_wiki`, and `docs/artifact` are the **single source for the four cross-service contracts** OAuth / image hosting / galgame-wiki / artifact (large files). The `docs/{oauth,image_service,galgame_wiki,artifact}` in the forum / patch repos are **banner-stamped mirrors generated by kungal-docs's `pnpm docs:sync`**; **do not hand-edit the downstream copies** (the next sync will overwrite them).
...
- The **source of truth for these contracts is in the code** (`cmd/oauth`, `cmd/image`, `cmd/galgame`, `cmd/artifact`, and other handlers) — when you change the code, change the source docs here in the same PR; `docs:verify` will catch "docs that don't match the reality of the code".
